...
- **جلب الخطوط أثناء البناء.** يُحمِّل kou مُسبقًا خطًّا واحدًا لكل كتابة — Fira
  Code للاتينية، وSource Han Sans للمقاطع CJK، وNoto Naskh Arabic للعربية، و
  Noto Sans Devanagari، وNoto Sans Thai — إلى ذاكرة تخزين مؤقت مشتركة عند وقت
  البناء. تجاوز العائلات أو ثبّت ملفات محلية عبر متغيرات البيئة؛ وجّه التنزيلات
  عبر وسيط HTTP(S) (يُمرَّر إلى reqwest) عند التواجد خلف شبكة مقيَّدة. تُنقَّط
  المحارف باستخدام `ab_glyph`، مع تجربة كل وجه بالترتيب، فيمزج التصيير الواحد
  بين النصوص دون فراغات ناقصة (tofu).
