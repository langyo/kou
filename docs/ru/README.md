...
<p align="center"><strong>Автоматизация виртуального терминала — PTY + настоящий экран VT100 + шрифты в стиле ort + встроенные графические протоколы.</strong></p>
...
- **Настоящий экран.** Поток байтов обрабатывается парсером [`vte`](https://crates.io/crates/vte),
  ...
  раннего прототипа.
- **Шрифты в стиле ort.** kou не поставляется со шрифтами; он загружает
...
| `KOU_FONT_CJK` | `sarasa` (по умолчанию) / `sourcehansans` / `smileysans` / `none` |
