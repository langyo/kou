...
- **Получение шрифтов во время сборки.** kou предварительно загружает по одному
  шрифту на письменность — Fira Code для латиницы, Source Han Sans для CJK,
  Noto Naskh Arabic для арабского, Noto Sans Devanagari, Noto Sans Thai — в
  общий кеш во время сборки. Переопределяйте семейства или фиксируйте локальные
  файлы через переменные окружения; направляйте загрузки через HTTP(S) прокси
  (передаётся в reqwest), находясь за сетью с ограничениями. Глифы растеризуются
  с помощью `ab_glyph`, перебирая каждое начертание по порядку, что позволяет
  смешивать письменности в одном рендере без «тофу».
