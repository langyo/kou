...
- **建構時字型預取。** kou 會在建構時為每種書寫系統各預先下載一個字型——拉丁文用 Fira Code、CJK 用 Source Han Sans、阿拉伯文用 Noto Naskh Arabic、Noto Sans Devanagari、Noto Sans Thai——並存入共用快取。可透過環境變數覆寫字型家族或指定本地檔案；身處受限網路時，可透過 HTTP(S) 代理伺服器（傳遞給 reqwest）路由下載。字形由 `ab_glyph` 進行點陣化，依序嘗試每個字型面，因此單次渲染即可混合多種書寫系統而不會出現缺字（tofu）。
...
在建構時透過 `KOU_FONT_PRIMARY` / `KOU_FONT_CJK` / `KOU_FONT_ARABIC` /
`KOU_FONT_DEVANAGARI` / `KOU_FONT_THAI` 覆寫任意字型家族，或透過
`KOU_FONT_*_PATH` 指定本地檔案。下載可透過 `KOU_DOWNLOAD_PROXY`（直接傳遞給
reqwest）經由 HTTP(S) 代理伺服器路由。

| 環境變數 | 用途 |
|-----|---------|
| `KOU_FONT_PRIMARY` | 覆寫 Latin 字型家族。 |
| `KOU_FONT_CJK` | 覆寫 / 停用 CJK 字型（`none` 為停用）。 |
| `KOU_FONT_ARABIC` | 覆寫 / 停用阿拉伯文字型。 |
| `KOU_FONT_DEVANAGARI` | 覆寫 / 停用天城文字型。 |
| `KOU_FONT_THAI` | 覆寫 / 停用泰文字型。 |
