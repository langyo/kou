...
fn neofetch_showcase() {
    ...
    let info: [(&str, &str); 14] = [
        ...
        ("Font fetch","Async reqwest + OS font discovery (recursive)"),
    ...
    let line0 = format!("\x1b[1;36m{:<36}\x1b[0m  \x1b[1;37m{}\x1b[0m\x1b[2;37m@kou\x1b[0m\n", art[0], "langyo");
    ...
    sc.feed(b"\n  ");
    for c in 0u8..8 { sc.feed(format!("\x1b[4{}m    \x1b[0m", c).as_bytes()); }
    sc.feed(b"\n  ");
    for c in 8u8..16 { sc.feed(format!("\x1b[4{}m    \x1b[0m", c).as_bytes()); }
    sc.feed(b"\n");
...
fn rainbow_gradient() {
    ...
    let colors = [31u8, 31, 33, 33, 32, 32, 36, 36, 34, 34, 35, 35, 31, 33, 32, 36, 34, 35];
    sc.feed(b"\n\n");
...
fn protocol_comparison() {
    ...
    sc.feed("\x1b[1;37m\x1b[44m\x1b[K  Inline Image Protocols — Feature Comparison  \x1b[0m\n\n".as_bytes());
    let rows: &[(&str, &str, &str, &str); 10] = &[
        ...
        ("Cross-feed", "Sliding apc_buf",       "Sliding apc_buf",        "Sliding apc_buf"),
        ("Aspect",     "Contain-fit centred",   "Contain-fit centred",    "Pixel-fit"),
        ("Max size",   "Unlimited (chunked)",   "~512 KB (single frame)", "Unlimited (RLE)"),
        ("Dependency", "Built-in",              "Built-in",               "icy_sixel crate"),
        ("Cargo feat", "default",               "default",                "sixel"),
        ("Terminals",  "kitty, wezterm, ghostty","iTerm2, wezterm",       "xterm, mlterm, mintty"),
    ];
...
fn inline_image_kitty2() {
    let mut sc = Screen::new(120, 60);
    sc.feed("\x1b[1;37m\x1b[44m\x1b[K  Inline Image Rendering — Kitty2 APC Protocol  \x1b[0m\n\n".as_bytes());

    // Load the kou logo, build a kitty APC, feed it.
    let logo_png = std::fs::read("docs/logo.webp").ok()
        .and_then(|webp| {
            ...
            image::codecs::png::PngEncoder::new(&mut buf)
                .write_image(img.as_rgba8()?.as_raw(), img.width(), img.height(), image::ExtendedColorType::Rgba8)
                .ok()?;
            Some(buf)
        })
        .unwrap_or_else(|| {
            ...
            let img: ImageBuffer<Rgba<u8>, Vec<u8>> = ImageBuffer::from_pixel(64, 64, Rgba([100, 150, 200, 255]));
            let mut buf = Vec::new();
            image::codecs::png::PngEncoder::new(&mut buf)
                .write_image(img.as_raw(), 64, 64, image::ExtendedColorType::Rgba8).unwrap();
            buf
        });
    ...
    sc.feed("\x1b[28;8H\x1b[32m● Decoded from PTY stream → InlineImageStore → rendered\x1b[0m".as_bytes());
    sc.feed("\x1b[30;8H\x1b[32m● Contain-fit: aspect ratio preserved, centred\x1b[0m".as_bytes());
    sc.feed("\x1b[32;8H\x1b[32m● Cross-feed sliding buffer handles chunked transfers\x1b[0m".as_bytes());
    ...
    snapshot(&sc, "solarized-dark", "res/inline_image_kitty2_solarized_dark.png");
...
fn inline_image_iterm2() {
    let mut sc = Screen::new(120, 60);
    sc.feed("\x1b[1;37m\x1b[44m\x1b[K  Inline Image Rendering — iTerm2 OSC 1337  \x1b[0m\n\n".as_bytes());
    ...
    let logo_png = std::fs::read("docs/logo.webp").ok()
        ...
        .unwrap_or_else(|| {
            use image::{ImageBuffer, Rgba, ImageEncoder};
            let img: ImageBuffer<Rgba<u8>, Vec<u8>> = ImageBuffer::from_pixel(64, 64, Rgba([200, 100, 150, 255]));
            let mut buf = Vec::new();
            image::codecs::png::PngEncoder::new(&mut buf)
                .write_image(img.as_raw(), 64, 64, image::ExtendedColorType::Rgba8).unwrap();
            buf
    ...
    let b64 = base64::engine::general_purpose::STANDARD.encode(&logo_png);
    sc.feed(b"\x1b[4;8H");
    let iterm = format!("\x1b]1337;File=inline=1;width=20cells;height=20cells:{}\x07", b64);
    sc.feed(iterm.as_bytes());

    sc.feed("\x1b[26;8H\x1b[33miTerm2 OSC 1337\x1b[0m  \x1b[2m(ESC ] 1337;File= … BEL)\x1b[0m".as_bytes());
    ...
    snapshot(&sc, "solarized-dark", "res/inline_image_iterm2_solarized_dark.png");
}
...
fn classic_terminal() {
    ...
    let pal = (0..7).map(|c| format!("\x1b[3{}m█\x1b[0m", c)).collect::<String>();
    sc.feed(format!("  {}  R G B C M Y K\n", pal).as_bytes());
    sc.feed("\n".as_bytes());
    sc.feed("  \x1b[38;2;255;107;157m简体中文\x1b[0m · \x1b[38;2;107;255;157m日本語\x1b[0m\n".as_bytes());
    sc.feed("\n".as_bytes());
...
fn seia_search_results() {
    ...
    sc.feed("  \x1b[1m$\x1b[0m seia search \"rust async patterns\" --engine duckduckgo --limit 8\n".as_bytes());
    sc.feed("  \x1b[2;36mEngine:\x1b[0m \x1b[36mDuckDuckGo\x1b[0m  \x1b[2m| Results: 8 | Time: 0.42s\x1b[0m\n\n".as_bytes());
    let results = [
        ("Asynchronous Programming in Rust", "rust-lang.org/async-book", "The official guide to async/await. Covers futures, tokio, async-std, and common patterns for building concurrent Rust applications."),
        ("Tokio Tutorial", "tokio.rs/tokio/tutorial", "Learn how to build asynchronous applications with Tokio, the most popular async runtime for Rust. Includes channels, spawning, and I/O."),
        ("Futures Explained in 200 Lines of Rust", "cfsamson.github.io/books-futures-explained", "Build a future executor from scratch to understand how async/await works under the hood. Clear, concise, and code-heavy."),
        ("Pin and Unpin in Rust", "blog.cloudflare.com/pin-and-unpin-in-rust", "A deep dive into self-referential structs, the Pin type, and why async Rust needs them. By Cloudflare's engineering team."),
        ("async-std Documentation", "async.rs", "An asynchronous version of the Rust standard library. Provides async file I/O, networking, timers, and more."),
        ("Smol — a small and fast async runtime", "github.com/smol-rs/smol", "A minimal, correct, and fast async runtime. Smol is designed to be simple to understand and easy to build on."),
        ("Async Rust Is Hard", "jacko.io/async-rust-is-hard", "An honest discussion of the pain points of async Rust, with practical advice for navigating them in real projects."),
        ("The Why of Async/Await in Rust", "yatima.io/blog/Async-Await", "Historical context and design rationale behind Rust's async/await syntax and the tokio ecosystem."),
    ];
...
fn shirabe_debug_server() {
    ...
    sc.feed("  \x1b[32m●\x1b[0m CDP:      \x1b[4;34mws://localhost:3001/devtools\x1b[0m\n\n".as_bytes());
    sc.feed("  \x1b[1mHTTP API Endpoints\x1b[0m\n\n".as_bytes());
    let endpoints = [
        ("GET",  "/health",          "Health check — returns {\"ok\":true}"),
        ("GET",  "/info",            "Browser version, viewport dimensions, current URL"),
        ("POST", "/navigate",        "Navigate to a URL: {\"url\":\"https://…\"}"),
        ("POST", "/back",            "Go back in browser history"),
        ("POST", "/forward",         "Go forward in browser history"),
        ("POST", "/screenshot",      "Capture viewport as base64 PNG: {\"selector\":\"…\",\"full_page\":true}"),
        ("POST", "/click",           "Click an element: {\"selector\":\"#button\"}"),
        ("POST", "/type",            "Type text: {\"selector\":\"input\",\"text\":\"hello\",\"submit\":true}"),
        ("POST", "/press",           "Press a key: {\"key\":\"Enter\"}"),
        ("POST", "/evaluate",        "Execute JavaScript: {\"expression\":\"document.title\"}"),
        ("GET",  "/dom",             "Query DOM: ?selector=div&attribute=class&computed=true"),
        ("GET",  "/a11y",            "Accessibility tree snapshot: ?selector=nav"),
        ("GET",  "/console",         "Console log entries: ?level=error"),
        ("POST", "/resize",          "Resize viewport: {\"width\":1280,\"height\":720}"),
    ];
    for (method, path, desc) in &endpoints {
        let mc = if *method == "GET" { "2;37" } else { "2;33" };
        sc.feed(format!("  \x1b[{}m{:<5}\x1b[0m \x1b[36m{:<16}\x1b[0m {}\n", mc, method, path, desc).as_bytes());
    }
    ...
    sc.feed("  \x1b[32m●\x1b[0m Extracted from tairitsu packager — hardened to stand alone\n".as_bytes());
